...
import torch
from .tool import mkdir  # Assuming 'tool.py' has mkdir, if not, import from utils
...
def add_train_argument(p):
    ...
    p.add_argument('--fp_type',type=str,choices=['morgan','mixed'],default='mixed',
                   help='The type of fingerprint. morgan or mixed.')
    p.add_argument('--use_3d_features', action='store_true',
                   help='Enable 3D features extraction and usage in GAT.')
    p.add_argument('--edge_dim', type=int, default=20,
                   help='Dimension of edge features for 3D in GAT.')
    p.add_argument('--gat_scale', type=float, default=0.5,
                   help='Scale factor for GAT and FPN fusion.')
        # ===== GAT parameters =====
    p.add_argument('--nhid', type=int, default=64,
                   help='Hidden size of GAT attention heads.')
    p.add_argument('--nheads', type=int, default=4,
                   help='Number of attention heads in GAT.')
    p.add_argument('--dropout_gat', type=float, default=0.1,
                   help='Dropout rate for GAT layers.')

    # ===== FPN parameters =====
    p.add_argument('--fp_2_dim', type=int, default=256,
                   help='Hidden dim after first FPN layer.')
    p.add_argument('--dropout', type=float, default=0.1,
                   help='Dropout rate for FPN part.')
