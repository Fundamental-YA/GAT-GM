# /home/abc/桌面/_______ph4_______/gatgm/tool/tool.py
import os
import csv
import logging
import math
...
from gatgm.data import MoleDataSet  # 如果其他函数需要
...
def split_data(data, type, size, seed, log):
    from gatgm.data import scaffold_split  # 延迟导入
    assert len(size) == 3
    assert sum(size) == 1
    if type == 'random':
...
def load_model(path, cuda, log=None, pred_args=None):
    ...
    # 设置 weights_only=False 以允许加载 argparse.Namespace
